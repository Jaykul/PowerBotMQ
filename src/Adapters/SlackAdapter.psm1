...
function Send-Message {
    ...
    process {
        ...
        Write-Verbose "Send-SlackMessage -To $to -Message $Message -From $From"
        if($Type -eq "Action") {
            $Message = "_${Message}_"
        }
...
function Start-Adapter {
    ...
    $Script:Network = $Network.Scheme + "://" + $Network.Host
    $Script:Channel = $Channel
    $Character = $Null
    while($Character -ne "Q") {
        while(!$Host.UI.RawUI.KeyAvailable) {
            foreach($envelope in PowerBotMQ\Receive-Message -NotFromNetwork $Network.Host -NotFromChannel $Channel -TimeoutMilliSeconds 2000) {
                ...
                if(!$envelope) { # we risk lagging off ...
                    Write-Debug "Ping: $($Client.PingRoundTripMilliseconds) IsConnected? $($Client.IsConnected)"
                    if(!$Client.IsConnected) {
                        Write-Warning "We are disconnected?! Try re-initializing"
                        $Script:Client = InitializeAdapter -Context $Context -Network $Network -Channel $Channel -Token $Token
                    }
                }
            }
